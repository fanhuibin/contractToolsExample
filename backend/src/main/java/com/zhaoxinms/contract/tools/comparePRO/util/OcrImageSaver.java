...
public class OcrImageSaver {
     ...
     * @return 保存的图片目录路径
     */
    public Path saveOcrImages(Path pdfPath, String taskId, String mode, CompareOptions options) throws Exception {
        // 调试日志：记录去水印设置
        System.out.println("[" + mode + "] OcrImageSaver收到的去水印设置: " + (options != null ? options.isRemoveWatermark() : "options为null"));
        
        // 检查是否启用图片保存功能
        if (!zxOcrConfig.isSaveOcrImages()) {
        ...
        try (PDDocument doc = PDDocument.load(pdfPath.toFile())) {
            ...
            long minPixels = zxOcrConfig.getMinPixels();
            long maxPixels = zxOcrConfig.getMaxPixels();
