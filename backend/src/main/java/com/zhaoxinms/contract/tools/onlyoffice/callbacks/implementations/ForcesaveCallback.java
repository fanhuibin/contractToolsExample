...
public class ForcesaveCallback implements Callback {
    ...
    @Override
    public int handle(Track body, String fileId, String contractId) { // handle the callback when the force saving request is performed
        int result = 0;
        try {
            ...
            log.error("强制保存回调处理失败: {}", ex.getMessage(), ex);
            result = 1;
        }
