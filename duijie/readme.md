...
### 2025-08-14 智能审核集成 OnlyOffice 风险提示

#### 功能概述
本次更新在智能审核流程中集成了 OnlyOffice，实现了在文档右侧实时显示风险提示的功能，并支持点击风险项定位到文档中的具体位置。

#### 关键特性
1. **新建审核结果详情页**
   - 创建了全新的 `ContractReviewDetail.vue` 页面，用于展示审核结果。
   - 页面布局分为三栏：顶部导航、左侧 OnlyOffice 文档显示、右侧风险提示面板。

2. **OnlyOffice 集成**
   - 在详情页中嵌入了 OnlyOffice 编辑器，用于实时显示和预览合同文档。
   - 支持通过路由参数传递文件 ID，动态加载不同文件。

3. **风险提示与定位**
   - 右侧面板实时展示审核发现的风险，包括风险名称、详细描述和修改建议。
   - 不同风险等级（高、中、低）使用不同颜色进行醒目标识。
   - 点击任意风险项，可调用 OnlyOffice API，自动在文档中高亮并跳转到问题语句所在位置，实现了书签式定位。

4. **交互流程优化**
   - 在原有的智能审核页面 (`ContractReview.vue`) 添加了"查看详情"按钮。
   - 用户完成审核后，可点击按钮跳转到新的详情页，查看带有文档上下文的风险报告。
   - 通过 `localStorage` 在页面跳转时传递审核结果数据，提升了用户体验的流畅性。

#### 技术实现

##### 主要文件变更
- **新增**: `frontend/src/views/contracts/ContractReviewDetail.vue` - 全新的审核结果详情页。
- **修改**: `frontend/src/views/contracts/ContractReview.vue` - 添加了"查看详情"按钮和跳转逻辑。
- **修改**: `frontend/src/router/index.ts` - 为新页面添加了路由配置 `/contract-review-detail/:fileId`。
- **新增**: `frontend/src/components/onlyoffice/plugins/RiskLocator.js` - 用于处理风险定位的OnlyOffice插件。

##### 核心实现
- **风险定位**: 利用 OnlyOffice 的 `postMessage` API，向插件发送定位指令，实现文档内的精准跳转和高亮。
- **数据持久化**: 在页面跳转期间，临时使用 `localStorage` 存储审核结果，确保新页面能获取到完整数据。
- **组件化**: 右侧风险面板被设计为一个独立的滚动区域，可以展示大量风险信息而不影响页面整体布局。

#### Bug 修复
- **修复了 `ContractReview.vue` 中 `router` 变量重复声明导致的编译错误**。移除了冗余的 `useRouter` 调用，确保了代码的整洁和正确性。
- **修复了文件不存在错误**。使用系统中已有的文件ID（tempalteDesign），确保OnlyOffice能够正确加载文档。
- **增强了风险定位功能**。改进了locateRisk方法，添加了错误处理和更详细的位置信息，提高了定位的准确性和稳定性。
- **优化了事件处理**。添加了文档状态变化处理方法，使系统能够更好地响应OnlyOffice编辑器的状态变化。