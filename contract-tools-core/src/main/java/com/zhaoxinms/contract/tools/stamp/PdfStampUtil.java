...
public class PdfStampUtil {
    ...
    public static void main(String[] args) {
        ...
        // 测试配置
        String testDir = "C:\\Users\\范慧斌\\Desktop\\testMerge";
        String inputPdf = testDir + "/test1.pdf";  // 测试PDF文件
        String stampImage = testDir + "/stamp.png"; // 印章图片
...
//            logger.info("\n--- 测试2：添加普通印章 ---");
//            if (new File(inputPdf).exists()) {
//                String outputStamp = testDir + "/test_normal_stamp.pdf";
//                addStamp(inputPdf, outputStamp, stampImage, 1, 400f, 200f);
//                logger.info("普通印章测试完成: {}", outputStamp);
//            }
//            
            // 测试3：自动识别盖章
            logger.info("\n--- 测试3：自动识别盖章 ---");
            if (new File(inputPdf).exists()) {
                ...
                // 用户需要提供关键词列表
                List<String> keywords = Arrays.asList("（公章）", "（签章）", "测试关键词");
