# License授权文件部署指南

## 📋 概述

本文档说明如何在生产环境中部署License授权文件，确保SDK打包后能够正确加载授权。

---

## 🎯 部署结构

### 推荐的生产环境部署结构

```
/opt/zhaoxin-contract-tools/
  ├── contract-tools-sdk-1.0.0.jar    # SDK主程序
  ├── license.lic                      # 授权文件（方式1：jar包同级）
  ├── publicCerts.store                # 公钥文件（方式1：jar包同级）
  ├── config/                          # 配置目录
  │   ├── application.yml              # 应用配置
  │   ├── license.lic                  # 授权文件（方式2：config目录）
  │   └── publicCerts.store            # 公钥文件（方式2：config目录）
  └── logs/                            # 日志目录
```

---

## 📂 文件加载优先级

### License文件 (`license.lic`)

系统按以下优先级查找授权文件：

1. **jar包同级目录** ✅ 推荐
   ```
   ./license.lic
   ```

2. **config目录**
   ```
   ./config/license.lic
   ```

3. **classpath**（开发环境）
   ```
   classpath:license.lic
   ```

### 公钥文件 (`publicCerts.store`)

与License文件使用相同的优先级规则：

1. **jar包同级目录** ✅ 推荐
   ```
   ./publicCerts.store
   ```

2. **config目录**
   ```
   ./config/publicCerts.store
   ```

3. **classpath**（开发环境）
   ```
   classpath:publicCerts.store
   ```

---

## 🚀 部署步骤

### 步骤1：构建SDK

```bash
cd contract-tools-sdk
mvn clean package -DskipTests
```

**生成文件**：`target/contract-tools-sdk-1.0.0.jar`

### 步骤2：创建部署目录

```bash
# Linux/Mac
mkdir -p /opt/zhaoxin-contract-tools/config
mkdir -p /opt/zhaoxin-contract-tools/logs

# Windows
mkdir C:\zhaoxin-contract-tools\config
mkdir C:\zhaoxin-contract-tools\logs
```

### 步骤3：复制文件

```bash
# 复制SDK jar包
cp target/contract-tools-sdk-1.0.0.jar /opt/zhaoxin-contract-tools/

# 方式1：放在jar包同级目录（推荐）
cp /path/to/license.lic /opt/zhaoxin-contract-tools/
cp /path/to/publicCerts.store /opt/zhaoxin-contract-tools/

# 或者方式2：放在config目录
cp /path/to/license.lic /opt/zhaoxin-contract-tools/config/
cp /path/to/publicCerts.store /opt/zhaoxin-contract-tools/config/
```

### 步骤4：配置文件（可选）

如果需要自定义配置，创建 `config/application.yml`：

```yaml
server:
  port: 8080

zhaoxin:
  auth:
    enabled: true
    license:
      file-path: classpath:license.lic  # 使用classpath前缀，会自动按优先级查找
    signature:
      public-key-path: classpath:publicCerts.store
```

### 步骤5：启动应用

```bash
cd /opt/zhaoxin-contract-tools

# 方式1：直接启动
java -jar contract-tools-sdk-1.0.0.jar

# 方式2：指定配置文件
java -jar contract-tools-sdk-1.0.0.jar --spring.config.location=./config/application.yml

# 方式3：后台运行
nohup java -jar contract-tools-sdk-1.0.0.jar > logs/app.log 2>&1 &

# 方式4：使用systemd服务（Linux）
systemctl start zhaoxin-contract-tools
```

---

## 🔄 更新授权文件

### 无需重启更新（推荐）

**优点**：授权文件在jar包外部，可以随时替换而无需重新打包。

**步骤**：

```bash
# 1. 停止应用
kill <pid>

# 2. 备份旧授权
cp license.lic license.lic.bak

# 3. 替换新授权
cp /path/to/new-license.lic ./license.lic

# 4. 重启应用
java -jar contract-tools-sdk-1.0.0.jar
```

### 验证新授权

```bash
# 测试授权API
curl http://localhost:8080/api/auth/license-info

# 检查日志
tail -f logs/app.log | grep -i license
```

**预期日志**：
```
从jar包同级目录加载文件: ./license.lic
从jar包同级目录加载文件: ./publicCerts.store
```

---

## 📝 配置说明

### application.yml 配置项

```yaml
zhaoxin:
  auth:
    enabled: true  # 是否启用授权验证
    license:
      file-path: classpath:license.lic  # 授权文件路径
    signature:
      public-key-path: classpath:publicCerts.store  # 公钥路径
```

### 路径格式支持

| 格式 | 示例 | 说明 |
|------|------|------|
| `classpath:` | `classpath:license.lic` | 自动按优先级查找（推荐） |
| 相对路径 | `./license.lic` | 相对于jar包所在目录 |
| 绝对路径 | `/etc/zhaoxin/license.lic` | 指定完整路径 |
| `file:` | `file:./license.lic` | 显式指定文件系统路径 |

---

## 🐳 Docker部署

### Dockerfile示例

```dockerfile
FROM openjdk:11-jre-slim

# 创建应用目录
WORKDIR /app

# 复制jar包
COPY target/contract-tools-sdk-1.0.0.jar app.jar

# 暴露端口
EXPOSE 8080

# 启动命令（授权文件通过volume挂载）
ENTRYPOINT ["java", "-jar", "app.jar"]
```

### docker-compose.yml示例

```yaml
version: '3.8'

services:
  contract-tools:
    build: .
    ports:
      - "8080:8080"
    volumes:
      # 挂载授权文件到容器内jar包同级目录
      - ./license.lic:/app/license.lic
      - ./publicCerts.store:/app/publicCerts.store
      # 挂载配置文件
      - ./config:/app/config
      # 挂载日志目录
      - ./logs:/app/logs
    environment:
      - JAVA_OPTS=-Xmx2g -Xms1g
```

### 启动Docker容器

```bash
# 构建镜像
docker build -t zhaoxin-contract-tools:1.0.0 .

# 运行容器
docker-compose up -d

# 查看日志
docker-compose logs -f
```

---

## 🔧 systemd服务配置（Linux）

### 创建服务文件

```bash
sudo vi /etc/systemd/system/zhaoxin-contract-tools.service
```

**内容**：

```ini
[Unit]
Description=Zhaoxin Contract Tools SDK
After=network.target

[Service]
Type=simple
User=zhaoxin
Group=zhaoxin
WorkingDirectory=/opt/zhaoxin-contract-tools
ExecStart=/usr/bin/java -jar /opt/zhaoxin-contract-tools/contract-tools-sdk-1.0.0.jar
Restart=on-failure
RestartSec=10
StandardOutput=append:/opt/zhaoxin-contract-tools/logs/app.log
StandardError=append:/opt/zhaoxin-contract-tools/logs/error.log

[Install]
WantedBy=multi-user.target
```

### 启用和启动服务

```bash
# 重新加载systemd配置
sudo systemctl daemon-reload

# 启用服务（开机自启）
sudo systemctl enable zhaoxin-contract-tools

# 启动服务
sudo systemctl start zhaoxin-contract-tools

# 查看状态
sudo systemctl status zhaoxin-contract-tools

# 查看日志
sudo journalctl -u zhaoxin-contract-tools -f
```

---

## 🔍 故障排查

### 问题1：找不到授权文件

**症状**：
```
License文件不存在: classpath:license.lic
```

**排查步骤**：

```bash
# 1. 检查文件是否存在
ls -la /opt/zhaoxin-contract-tools/license.lic
ls -la /opt/zhaoxin-contract-tools/config/license.lic

# 2. 检查文件权限
chmod 644 /opt/zhaoxin-contract-tools/license.lic

# 3. 检查工作目录
pwd  # 应该在jar包所在目录

# 4. 查看日志中的加载信息
grep "加载文件" logs/app.log
```

### 问题2：公钥文件读取失败

**症状**：
```
公钥未配置且公钥文件不存在: classpath:publicCerts.store
```

**解决方案**：

```bash
# 确保公钥文件存在
ls -la publicCerts.store

# 检查文件内容
head -n 5 publicCerts.store
# 应该看到: -----BEGIN PUBLIC KEY-----
```

### 问题3：授权验证失败

**症状**：
```
License签名验证失败
```

**原因**：
- 公钥与授权文件不匹配
- 授权文件被篡改

**解决方案**：

```bash
# 重新生成授权文件，确保使用配套的密钥对
cd contract-tools-auth-generator
java -jar target/contract-tools-auth-generator-1.0.0-jar-with-dependencies.jar
```

---

## 📊 日志示例

### 成功加载授权

```
2025-10-18 18:00:00 [main] INFO  c.z.c.t.a.s.LicenseService - 从jar包同级目录加载文件: ./license.lic
2025-10-18 18:00:00 [main] INFO  c.z.c.t.a.s.LicenseService - 从jar包同级目录加载文件: ./publicCerts.store
2025-10-18 18:00:01 [main] INFO  c.z.c.t.a.s.LicenseService - License验证成功
2025-10-18 18:00:01 [main] INFO  c.z.c.t.a.s.LicenseService - 授权单位: 测试公司
2025-10-18 18:00:01 [main] INFO  c.z.c.t.a.s.LicenseService - 授权有效期: 2025-01-18 ~ 2026-01-18
2025-10-18 18:00:01 [main] INFO  c.z.c.t.a.s.LicenseService - 授权模块: 6个
```

### 从config目录加载

```
2025-10-18 18:00:00 [main] INFO  c.z.c.t.a.s.LicenseService - 从config目录加载文件: ./config/license.lic
2025-10-18 18:00:00 [main] INFO  c.z.c.t.a.s.LicenseService - 从config目录加载文件: ./config/publicCerts.store
```

### 开发环境（classpath）

```
2025-10-18 18:00:00 [main] INFO  c.z.c.t.a.s.LicenseService - 从classpath加载文件: classpath:license.lic
2025-10-18 18:00:00 [main] INFO  c.z.c.t.a.s.LicenseService - 从classpath加载文件: classpath:publicCerts.store
```

---

## ⚠️ 重要提示

### 文件权限

```bash
# Linux/Mac权限设置
chmod 644 license.lic
chmod 644 publicCerts.store
chown zhaoxin:zhaoxin license.lic publicCerts.store
```

### 安全建议

1. **授权文件保护**：
   - 设置适当的文件权限（644或更严格）
   - 定期备份授权文件
   - 使用环境变量或安全工具管理敏感配置

2. **公钥文件**：
   - 可以打包到jar内（classpath）
   - 也可以外部部署，便于更新

3. **私钥文件**：
   - **绝对不要**部署到生产服务器
   - 仅在授权生成环境中使用

---

## 📞 技术支持

如有问题，请联系：

- **企业网址**：https://www.zhaoxinms.com
- **技术支持**：develop@zhaoxinms.com
- **授权咨询**：联系商务团队

---

**文档版本**：v1.0.0  
**最后更新**：2025-10-18  
**适用于**：contract-tools-sdk 1.0.0+

