...
GET    /api/compare/tasks/{taskId}           # 获取任务状态
GET    /api/compare/tasks/{taskId}/result    # 获取比对结果
DELETE /api/compare/tasks/{taskId}           # 删除任务
```
...
| `errorDetail` | String | ❌ | 详细错误信息（仅开发环境） |
...
```json
{
  ...
  "traceId": "abc123",
  "timestamp": "2025-01-18T10:30:00",
  "errorDetail": "NullPointerException at line 123..."
}
```
...
#### 推荐请求头
- `X-Trace-Id` - 请求追踪ID（客户端生成UUID）
- `Authorization` - 认证Token（如有）
...
每个请求都有唯一的TraceID：
- 客户端生成UUID通过`X-Trace-Id`请求头传递
- 服务端在响应头和响应体中返回TraceID
- 所有日志都包含TraceID，便于问题排查
