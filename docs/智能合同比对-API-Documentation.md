...
| `newFileName` | string | 新版本文档文件名 |
| `startTime` | string | 任务开始时间（ISO格式） |
| `endTime` | string | 任务结束时间（ISO格式，未完成时为null） |
| `differenceCount` | number\|null | 差异总数（仅已完成任务有值，未完成为null） |
| `resultUrl` | string\|null | 比对结果访问地址（仅已完成任务有值，未完成为null） |

> **说明**：此接口返回简化版本的任务列表，仅包含核心信息。如需获取完整的任务详情，请使用 `/task/{taskId}` 接口。
