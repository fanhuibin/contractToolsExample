# 前端集成指南

本文档详细说明如何在你的前端项目中集成肇新合同比对服务。

## 📋 目录

- [集成方式](#集成方式)
- [iframe 嵌套方案](#iframe-嵌套方案)
- [API 对接方案](#api-对接方案)
- [混合方案（推荐）](#混合方案推荐)
- [实战案例](#实战案例)

---

## 集成方式

肇新合同比对服务提供两种集成方式：

| 方式 | 优点 | 缺点 | 适用场景 |
|------|------|------|---------|
| **iframe 嵌套** | 零代码，快速集成 | 样式难以自定义 | 快速原型，内部系统 |
| **API 对接** | 完全自定义 | 开发工作量大 | 品牌化需求强 |
| **混合方案** | 平衡定制与开发成本 | 需要理解两种方式 | **推荐方案** |

---

## iframe 嵌套方案

### 1. 基础嵌套

#### 嵌套比对页面

```html
<iframe 
  src="http://localhost:3000/gpu-ocr-compare" 
  width="100%" 
  height="800px"
  frameborder="0">
</iframe>
```

#### 嵌套结果页面

```html
<iframe 
  src="http://localhost:3000/gpu-ocr-compare/canvas-result/{taskId}" 
  width="100%" 
  height="800px"
  frameborder="0">
</iframe>
```

### 2. 嵌入模式（隐藏导航栏）

为了让 iframe 内容更加纯净，可以使用**嵌入模式**：

```html
<iframe 
  src="http://localhost:3000/gpu-ocr-compare/canvas-result/{taskId}?embed=true" 
  width="100%" 
  height="800px"
  frameborder="0">
</iframe>
```

**效果**：
- ✅ 隐藏肇新前端的侧边栏
- ✅ 隐藏顶部导航栏
- ✅ 只显示核心比对结果

### 3. Vue 3 示例

```vue
<template>
  <div class="result-page">
    <iframe :src="iframeUrl" class="result-iframe" />
  </div>
</template>

<script setup>
import { computed } from 'vue'
import { useRoute } from 'vue-router'

const route = useRoute()
const taskId = computed(() => route.params.taskId)

const iframeUrl = computed(() => 
  `http://localhost:3000/gpu-ocr-compare/canvas-result/${taskId.value}?embed=true`
)
</script>

<style scoped>
.result-iframe {
  width: 100%;
  height: 100vh;
  border: none;
  display: block;
}
</style>
```

### 4. React 示例

```jsx
import { useParams } from 'react-router-dom';

function ResultPage() {
  const { taskId } = useParams();
  const iframeUrl = `http://localhost:3000/gpu-ocr-compare/canvas-result/${taskId}?embed=true`;
  
  return (
    <div className="result-page">
      <iframe 
        src={iframeUrl}
        className="result-iframe"
        style={{ width: '100%', height: '100vh', border: 'none' }}
      />
    </div>
  );
}

export default ResultPage;
```

### 5. Angular 示例

```typescript
import { Component, OnInit } from '@angular/core';
import { ActivatedRoute } from '@angular/router';
import { DomSanitizer, SafeResourceUrl } from '@angular/platform-browser';

@Component({
  selector: 'app-result',
  template: `
    <div class="result-page">
      <iframe [src]="iframeUrl" class="result-iframe"></iframe>
    </div>
  `,
  styles: [`
    .result-iframe {
      width: 100%;
      height: 100vh;
      border: none;
      display: block;
    }
  `]
})
export class ResultComponent implements OnInit {
  iframeUrl: SafeResourceUrl;

  constructor(
    private route: ActivatedRoute,
    private sanitizer: DomSanitizer
  ) {}

  ngOnInit() {
    const taskId = this.route.snapshot.paramMap.get('taskId');
    const url = `http://localhost:3000/gpu-ocr-compare/canvas-result/${taskId}?embed=true`;
    this.iframeUrl = this.sanitizer.bypassSecurityTrustResourceUrl(url);
  }
}
```

### 6. 纯 HTML 示例

```html
<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>比对结果</title>
  <style>
    body, html {
      margin: 0;
      padding: 0;
      height: 100%;
      overflow: hidden;
    }
    .result-iframe {
      width: 100%;
      height: 100vh;
      border: none;
      display: block;
    }
  </style>
</head>
<body>
  <iframe 
    id="resultFrame"
    class="result-iframe">
  </iframe>

  <script>
    // 从 URL 获取 taskId
    const urlParams = new URLSearchParams(window.location.search);
    const taskId = urlParams.get('taskId');
    
    // 设置 iframe src
    const iframe = document.getElementById('resultFrame');
    iframe.src = `http://localhost:3000/gpu-ocr-compare/canvas-result/${taskId}?embed=true`;
  </script>
</body>
</html>
```

---

## API 对接方案

### 1. 核心流程

```
用户上传文件 → 调用比对API → 轮询状态 → 获取结果 → 展示结果
```

### 2. API 端点

| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/compare-pro/submit-url` | POST | 提交比对任务 |
| `/api/compare-pro/task/{taskId}` | GET | 查询任务状态 |
| `/api/compare-pro/canvas-result/{taskId}` | GET | 获取比对结果 |
| `/api/compare-pro/tasks` | GET | 获取任务列表 |
| `/api/compare-pro/export-report` | POST | 导出报告 |

详细 API 说明请参考 [API对接说明.md](./API对接说明.md)

### 3. Axios 客户端示例

```javascript
import axios from 'axios'

const client = axios.create({
  baseURL: 'http://localhost:8080',
  timeout: 60000
})

export default {
  // 提交比对任务
  async submitCompare(oldFileUrl, newFileUrl, options = {}) {
    const { data } = await client.post('/api/compare-pro/submit-url', {
      oldFileUrl,
      newFileUrl,
      removeWatermark: options.removeWatermark || false,
      oldFileName: options.oldFileName || '',
      newFileName: options.newFileName || ''
    })
    return data.data // taskId
  },

  // 查询任务状态
  async getTaskStatus(taskId) {
    const { data } = await client.get(`/api/compare-pro/task/${taskId}`)
    return data.data
  },

  // 获取比对结果
  async getResult(taskId) {
    const { data } = await client.get(`/api/compare-pro/canvas-result/${taskId}`)
    return data.data
  },

  // 获取任务列表
  async getAllTasks() {
    const { data } = await client.get('/api/compare-pro/tasks')
    return data.data
  }
}
```

### 4. 轮询任务状态

```javascript
async function pollTaskStatus(taskId, onProgress) {
  const maxAttempts = 60 // 最多轮询 60 次
  const interval = 2000  // 每 2 秒轮询一次
  
  for (let i = 0; i < maxAttempts; i++) {
    const status = await api.getTaskStatus(taskId)
    
    if (onProgress) {
      onProgress(i / maxAttempts, status)
    }
    
    if (status.status === 'COMPLETED') {
      return status
    }
    
    if (status.status === 'FAILED') {
      throw new Error(status.errorMessage || '任务失败')
    }
    
    await new Promise(resolve => setTimeout(resolve, interval))
  }
  
  throw new Error('任务超时')
}
```

---

## 混合方案（推荐）

**自定义上传页面 + iframe 嵌套结果页**

### 优势

- ✅ **上传页面完全自定义** - 符合品牌风格
- ✅ **结果页快速集成** - 零开发成本
- ✅ **维护成本低** - 肇新升级自动同步

### 架构图

```
┌─────────────────┐
│   你的前端      │
│                 │
│  ┌───────────┐  │
│  │ 上传页面  │  │ ← 自定义 UI
│  └─────┬─────┘  │
│        │        │
│        ↓ 提交   │
│  ┌───────────┐  │
│  │ Demo后端  │  │ ← API 代理
│  └─────┬─────┘  │
│        │        │
│        ↓ 转发   │
│  ┌───────────┐  │
│  │肇新后端   │  │ ← 核心服务
│  └─────┬─────┘  │
│        │        │
│        ↓ taskId │
│  ┌───────────┐  │
│  │ 结果页面  │  │ ← iframe 嵌套
│  │  (iframe) │  │    肇新前端
│  └───────────┘  │
└─────────────────┘
```

### 实现步骤

#### 步骤1：自定义上传页面

```vue
<template>
  <div class="upload-page">
    <h1>合同比对</h1>
    
    <!-- 文件上传 -->
    <input type="file" @change="handleOldFile" accept=".pdf,.doc,.docx">
    <input type="file" @change="handleNewFile" accept=".pdf,.doc,.docx">
    
    <!-- 提交按钮 -->
    <button @click="handleSubmit" :disabled="!canSubmit">
      开始比对
    </button>
    
    <!-- 进度显示 -->
    <div v-if="comparing">
      <progress :value="progress" max="100"></progress>
      <p>{{ statusText }}</p>
    </div>
  </div>
</template>

<script setup>
import { ref } from 'vue'
import { useRouter } from 'vue-router'
import api from '@/api'

const router = useRouter()
const oldFile = ref(null)
const newFile = ref(null)
const comparing = ref(false)
const progress = ref(0)
const statusText = ref('')

const handleOldFile = (e) => {
  oldFile.value = e.target.files[0]
}

const handleNewFile = (e) => {
  newFile.value = e.target.files[0]
}

const handleSubmit = async () => {
  comparing.value = true
  
  try {
    // 1. 上传文件
    progress.value = 20
    statusText.value = '上传文件中...'
    const oldUrl = await uploadFile(oldFile.value)
    const newUrl = await uploadFile(newFile.value)
    
    // 2. 提交比对
    progress.value = 40
    statusText.value = '提交比对任务...'
    const taskId = await api.submitCompare(oldUrl, newUrl)
    
    // 3. 轮询状态
    progress.value = 60
    statusText.value = '比对中...'
    await pollTaskStatus(taskId, (prog) => {
      progress.value = 60 + prog * 40
    })
    
    // 4. 跳转到结果页
    router.push(`/result/${taskId}`)
    
  } catch (error) {
    alert('比对失败: ' + error.message)
  } finally {
    comparing.value = false
  }
}
</script>
```

#### 步骤2：结果页面（iframe）

```vue
<template>
  <div class="result-page">
    <iframe :src="resultUrl" class="result-iframe" />
  </div>
</template>

<script setup>
import { computed } from 'vue'
import { useRoute } from 'vue-router'

const route = useRoute()
const taskId = computed(() => route.params.taskId)

const resultUrl = computed(() => 
  `http://localhost:3000/gpu-ocr-compare/canvas-result/${taskId.value}?embed=true`
)
</script>

<style scoped>
.result-iframe {
  width: 100%;
  height: 100vh;
  border: none;
}
</style>
```

---

## 实战案例

本 Demo 项目采用的就是**混合方案**：

### 项目结构

```
frontend-vue/
├── src/
│   ├── views/
│   │   ├── Compare.vue     ← 自定义上传页面
│   │   └── Result.vue      ← iframe 嵌套结果页
│   └── api/
│       └── index.js        ← API 客户端
```

### 查看源码

- [Compare.vue](../frontend-vue/src/views/Compare.vue) - 上传页面实现
- [Result.vue](../frontend-vue/src/views/Result.vue) - 结果页面实现
- [api/index.js](../frontend-vue/src/api/index.js) - API 客户端实现

---

## 🔧 配置说明

### 环境变量

建议使用环境变量管理不同环境的配置：

```javascript
// config.js
export const ZHAOXIN_CONFIG = {
  frontendUrl: import.meta.env.VITE_ZHAOXIN_FRONTEND_URL || 'http://localhost:3000',
  backendUrl: import.meta.env.VITE_ZHAOXIN_BACKEND_URL || 'http://localhost:8080'
}
```

```.env.development
VITE_ZHAOXIN_FRONTEND_URL=http://localhost:3000
VITE_ZHAOXIN_BACKEND_URL=http://localhost:8080
```

```.env.production
VITE_ZHAOXIN_FRONTEND_URL=https://zhaoxin.example.com
VITE_ZHAOXIN_BACKEND_URL=https://api.zhaoxin.example.com
```

---

## 📝 注意事项

### 1. CORS 配置

如果你的前端和肇新服务不在同一域名，需要在肇新后端配置 CORS：

```java
@Configuration
public class WebConfig implements WebMvcConfigurer {
    @Override
    public void addCorsMappings(CorsRegistry registry) {
        registry.addMapping("/api/**")
                .allowedOrigins("https://your-domain.com")
                .allowedMethods("GET", "POST", "PUT", "DELETE")
                .allowCredentials(true);
    }
}
```

### 2. 文件大小限制

确保你的服务器支持大文件上传（默认 50MB）：

```yaml
spring:
  servlet:
    multipart:
      max-file-size: 50MB
      max-request-size: 50MB
```

### 3. 生产环境部署

- 使用 HTTPS 协议
- 配置反向代理（Nginx）
- 启用 Gzip 压缩
- 设置合理的超时时间

---

## 🎓 最佳实践

1. **统一配置管理** - 所有 URL 集中在配置文件
2. **错误处理** - 提供友好的错误提示
3. **加载状态** - 显示进度条和状态信息
4. **响应式设计** - 支持移动端
5. **安全性** - 验证文件类型和大小

---

## 📞 技术支持

如有问题，请参考：
- [API对接说明](./API对接说明.md)
- [快速开始](./快速开始.md)
- 或联系肇新技术支持团队


